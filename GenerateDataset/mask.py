...
class FaceMasker:
    ...
    def mask(self):
        ...
        if found_face:
            ...
            toSave = 'fairface/Images14KWithMask/' + fileName.split('.')[0] + '-WithMask.' + fileName.split('.')[1]
            cv2.imwrite(toSave,image)
            print('No face found')
            faceNot.append(fileName.split('.')[0] + '-WithMask.' + fileName.split('.')[1])
            global countForNoFaceDetected
            countForNoFaceDetected += 1
    ...
    def _save(self):
        ...
        # print(fileName.split('.')[0] + '-WithMask.' + fileName.split('.')[1])
        # new_face_path = 'fairface/Images14KOnlyWithMask/' + fileName.split('.')[0] + '-WithMask.' + fileName.split('.')[1]
        new_face_path = fileName.split('.')[0] + '-WithMask.' + fileName.split('.')[1]
        faceFound.append(fileName.split('.')[0] + '-WithMask.' + fileName.split('.')[1])
...
# df = pd.read_csv ('/home/vaibhav/Desktop/Project/Data_Generator/14K.csv')
# df = df[:10]
# file_names = df['file']
...
#     countForNoFaceDetected = 0
#     for i in file_names:
#         image_path = 'fairface/' + i
#         create_mask(image_path)
        # print(i)
    # csvfile=open('noFace.csv','w', newline='')
    # obj=csv.writer(csvfile)
    # for image in  faceNot:
    #     obj.writerow([image])
    # csvfile.close()
    # csvfile=open('faceFound.csv','w', newline='')
    # obj=csv.writer(csvfile)
    # for image in  faceFound:
    #     obj.writerow([image])
    # csvfile.close()
    # csvfile=open('temp.csv','w', newline='')
    # obj=csv.writer(csvfile)
    # for row in df:
    #     # if row['file'] not in faceNot :
    #     print(row)

    # print(faceNot)

    # csvfile.close()
    # len('len of array',faceNot)

    # print('face not detected',countForNoFaceDetected)
imagepath1 = '1.jpg'
imagepath2 = '2.jpg'
imagepath3 = '3.jpg'
imagepath4 = '4.jpg'
imagepath5 = '6.jpg'
create_mask(imagepath1)
create_mask(imagepath2)
create_mask(imagepath3)
create_mask(imagepath4)
create_mask(imagepath5)